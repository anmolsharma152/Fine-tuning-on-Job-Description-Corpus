#!/usr/bin/env python3
"""
generate_instructions_test.py
TEST SCRIPT — same as generate_instructions.py but uses Llama 3.1 8B
for faster throughput comparison against Nemotron-Super 49B.

Default model: meta/llama-3.1-8b-instruct
Default output: data/test_llama8b.jsonl

Usage:
    python data/generate_instructions_test.py --count 50 --workers 10
"""
...
def main():
    parser = argparse.ArgumentParser(description="[TEST] Generate instruction data via Llama 3.1 8B")
    parser.add_argument("--count", type=int, default=50, help="Number of records to generate")
    parser.add_argument("--output", type=str, default="data/test_llama8b.jsonl", help="Output JSONL path")
    ...
    print(f"[TEST] Generating up to {args.count} records (batch_size={args.batch_size}, workers={args.workers})...")
    ...
    total = existing_count + len(all_records)
    rec_per_sec = (len(all_records) / elapsed) if elapsed > 0 else 0
    print(f"\nDone! {total} records saved to {args.output}")
    print(f"Elapsed: {elapsed:.1f}s  |  Throughput: {rec_per_sec:.2f} rec/s")
